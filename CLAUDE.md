...
cron 行の書き方は README にある（`README.ja.md`「複数の Goal を同時に回す」、`README.md`
"Running several Goals at once"）。ent 自身を直す Goal では、後ろの `mise run ent -- run` の
例を使う。並べ方は次の節の制約に従う。
...
この形にあたるのは、README の同じ節が載せている並列のレシピ（`for slug … &`）になる。
同じ節の cron の書き方は外部ロックで重ならないようにしてあるので、ここには当たらない
（ロックの規範は前の節に書いた）。
