...
- 複数 Actor の並列実行（インターフェースは複数対応、実装は逐次1本）。
  ...
  **ここで言う並列は1ティックの内側の話になる。** Goal単位のleaseがあるためデータモデル上は
  N本を別プロセスで扱えるが、同一ディレクトリでの並列実行は現在未対応。
  controller 自身の書き込みで状態DBの指紋が変わる分は説明が付くようにしたが（§10-6）、
  別プロセスの書き込みが起こす checkpoint はそこに入らず、保護パス違反として止まる。
  README「複数のGoalを同時に回す」のとおり、この制約が解けるまでは1本ずつ回す
...
**状態 DB だけは「誰が書いたか」を分ける。** `.goals/.state/goals.db` は
`outOfSightState` が指紋で見る保護対象でありながら、**controller 自身の書き込み先**
でもある。ACT の窓——ベースラインを控えてから検査するまでの間——で、controller は
必ずこの DB に書く——Run の write-ahead（`startRun`）と確定（`finishRun`）、そして
lease の延長になる。SQLite は WAL なので、その書き込みは普段 `goals.db-wal` に載るだけで
`goals.db` の中身は1バイトも動かない。自分のプロセスが接続を開いたままのこの窓では、
動くのは **WAL が既定の閾値（1000 ページ）を越えたコミットで自動 checkpoint が走り、
WAL の内容が `goals.db` へ畳み込まれたとき**になる。
ティックの形が同じでも、そのプロセスがそれまでに書いた量が閾値を跨いだ回だけ
`goals.db` の指紋が変わり、`ESCALATE(protected_path_touched)` になっていた。
人間も Actor も触っていないのに関門が鳴り、実装役の成果が publish されないまま
worktree に残る。

**区別すべきは「保護するかどうか」ではない。** `.goals/.state/**` は
`PROTECTED_PATH_FLOOR` に残す。外せば、DB を直接書き換えて状態を偽造されても
関門が鳴らない。落とす条件は「保護対象かどうか」ではなく
「その差分が controller 自身の書き込みで説明できるか」にする。説明が付くと言えるのは、
**自分の書き込みの前後で控えた指紋の連鎖が1度も切れていないとき**だけになる。
控え（比較の基準として保持している指紋）の起点は ACT 前のベースラインで、自分が書く
**直前**の指紋が控えと違えば（＝自分より先に誰かが書いた）そのティックのあいだ説明を
やめ、書いた**直後**の指紋を新しい控えにする。検査時の指紋が控えと一致した差だけを
落とす。起点をベースラインにするのが要で、「最後の書き込みの直後に取り直す」形に
すると、そこまでの間に書き換えられた内容が控えに畳み込まれて改竄に説明が付く。
指紋を取れなかった回は説明に使わない（§3.1。この扱いは下の (f) の `unreadable` と
同じ判定になる）。

**もう1本の controller が同じディレクトリで回している場合も、説明は付かない。**
あちらの書き込みも WAL を進めるので checkpoint を起こしうるが、それは
「controller 自身の書き込み」ではないので `protected_path_touched` のまま止まる。
いまはそこで鳴る。鳴らさずに済ませるには関門の側に手を入れる必要があり、
同じディレクトリから ent を並列で回さない運用はそれまでの制約になる（CLAUDE.md）。
...
パスで、そこを書き換えられても検知しない（`goals.db` 自身にも、WAL に留まる書き換えと
いう別の穴がある。(g)）。
...
(f) controller 自身の書き込みの直前と直後に取る指紋の間——ミリ秒の窓——に
他の誰かが状態 DB を書くと、その内容が控えに入って説明が付く。書き続ける相手は
次の書き込みの直前の確認で必ず捕まる（連鎖は1ティックのあいだ切れたままになる）ので、
抜けられるのはこの窓に1回だけ当てた場合になる。
ただし**「読めない」だけはこの窓でも説明しない**。controller の書き込みが
`fingerprint` の `unreadable` を作ることはないので、DB を消す・読めなくする側は
窓に当たっても止まる。あちらは一度当てれば状態が残り続けるため、
タイミングで結果が変わってはいけない。

(g) 状態 DB の改竄も指紋で見るのは `goals.db` だけで、`goals.db-wal` は見ていない。
WAL の中身は次の checkpoint まで `goals.db` に現れないので、SQLite 経由で
書き換えられた行は、そのティックの指紋の差には出ないことがある。
