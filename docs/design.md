...
でもある。ACT の前後で指紋を比べる窓の中で、controller は必ずこの DB に書く——
Run の write-ahead（`startRun`）と確定（`finishRun`）、そして lease の延長になる。
SQLite は WAL なので、その書き込みは普段 `goals.db-wal` に載るだけで `goals.db` の
中身は1バイトも動かない。動くのは **WAL が既定の閾値（1000 ページ）を越えた
コミットで自動 checkpoint が走り、WAL の内容が `goals.db` へ畳み込まれたとき**になる。
...
**外したのは保護ではなく、説明の付く差になる。** `.goals/.state/**` は
...
ACT 前のベースラインを連鎖の起点とし、自分が書く**直前**の指紋が控えと違えば
（＝自分より先に誰かが書いた）そのティックのあいだ説明をやめ、書いた**直後**の指紋を
新しい控えにする。検査時の指紋が控えと一致した差だけを落とす。
起点をベースラインにするのが要で、「最後の書き込みの直後に取り直す」形にすると、
そこまでの間に書き換えられた内容が控えに畳み込まれて改竄に説明が付く。
指紋を取れなかった回は説明に使わない（§3.1）。
...
もう1本の controller が同じディレクトリで回している場合、あちらの書き込みも
WAL を進めるので checkpoint を起こしうるが、それは「controller 自身の書き込み」では
ないので説明が付かず、`protected_path_touched` のまま止まる。**鳴るのが正しい。**
同じディレクトリから ent を並列で回さないことは CLAUDE.md が別に決めている。
