...
言い切るティック（`COMPLETE` と `WAIT`）で `local.dirty` を読み、汚れていれば
`ESCALATE(uncommitted_changes)` にする（§10-11）。
...
11. ~~**「Actor が commit する」という前提を誰が確かめるか**~~ — 確定した。
    controller が ACT の外側で確かめ、確かめられなければ
    `ESCALATE(uncommitted_changes)` にする。
    ...
    **材料は既にあった `local.dirty` を読む。** 観測を足していない。VERIFY と同じく
    ...
    確かめられなかったティックは違反にしない。捏造した違反で人間を呼ぶと、
    関門そのものが信用されなくなる（§3.1）。
    **逆向きの誤検知を2つ避ける。** Gap が残っているティック（実装の途中で作業ツリーが
    汚れているのは正常）は `ACT` なので関門を通らない。Actor がまだ1度も走っていない
    Goal は、そもそも見ない。1ティック目は worktree が無く `local.*` は controller 自身の
    リポジトリを観測するので（§10-9）、自己ホストでは人間の編集で汚れているのが普通に
    なる。それを Actor の書き残しと読むと、どの Goal も最初のティックから進まない。
