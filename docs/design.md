...
最終更新: 2026-08-11（タスク分解の粒度を決め、分解した1本ごとに Goal を立てる方針にして、順序の宣言 `goal.depends_on` を入れた。分解を機械にやらせる場合の宣言部の書き手も決めた。実装は保留）
...
12 が残す2つは、入った `goal.depends_on` の穴で、粗いタスクを複数の Goal に割って
回すときに効く。自己ホストはまだ `depends_on` を使っていないので、ここには数えない。
12 はもう1つ、分解を機械にやらせる場合に criteria の変更を検知する指紋を保留にしている。
経路そのものがまだ無いので、これも数えない。
以下は実運用で必要になった順に埋める。
...
11. ~~**「Actor が commit する」という前提を誰が確かめるか**~~ — 確定した。
    ...
    **その「出てよい」を実際に出せるようにするため、push の機会を Actor の実行から
    外した。** この関門の解決手順は人間が commit することで、**その commit には Run が
    付かない。** `publish` は「完了した Run が無いティックでは push しない」を条件に
