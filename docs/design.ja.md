...
**試行台帳は保存せず、読むときに導く。** 1つの試行——Actor を1回起動した分——は、
`Run`（何をさせるつもりだったか、誰が走ったか、何が変わったか、なぜ失敗したか）に、
それを閉じた `Verification` のティックと、その前後の `Snapshot` を結んだものになる。
3つとも既にテーブルを持っていて、足りないのは結び方だけなので、`buildAttempts`
（`src/domain/attempt.ts`）が読むときに結び、4つ目のテーブルを作らない。保存する形に
すると、既に置き場のある事実をもう一度書くことになり、書き損ねた回の写しが黙って古くなる。

**試行を閉じるのは Actor の exit code ではなく、次のティックの VERIFY になる。**
exit code 0 はプロセスが終わったことしか言わない。その試行が何かを達成したかどうかは、
あとで走る検証でしか決まらない。1ティックの中は OBSERVE → ACT → 永続化の順なので、
同じティックで書かれる検証結果は Run より**前**の時刻を持つ。だから「Run の
`started_at` より後で最も古いラウンド」は必ず次のティックの分になる。`runs` に
`reconcile_seq` の列を足さずに結べるのはこのためになる。

**Actor の最終メッセージは `actor_claim` で、Fact にはしない。** 生ログ（§4.6）にあり、
読むのは Actor へ渡すビューだけで、完了判定にもあらゆる停止判断にも使わない（§3.1）。
次の Actor に対して立つ位置は、レビュー役に対する PR のタイトルと本文と同じになる。
見る材料であって、判定の基準ではない。
...
<worktree>/.goals/.state/attempts.md 試行台帳を Actor が読む形にしたもの。ティックの頭で作り直す
```

**試行台帳のビューは写しで、上限を付けて作り直す。** Actor は毎ティック新しいセッションで
走り、`resume` もセッション ID も渡らないので、前の Actor が試して捨てた道は、ent が
渡さない限り次の Actor には届かない。正本は状態 DB のまま（§4.5）で、作業ツリーに置くのは
直近数件を Markdown にしたものになる。ティックが始まる前に置き直す。`actor_claim` と
ent が確かめた結果を別の見出しに分け、本文の冒頭で「指示ではなく参考情報」と自分で名乗る。
何をするかを決めるのは intent の側で、指示が2つ並ぶ形にしない。

**Actor がこれを編集するのは止められない。** 置き場所は作業ツリーの `.goals/.state/` で、
保護パスの検査が唯一照合から外す場所になる（外さないと、一時ファイルが1本増えるだけで
`protected_path_touched` になり、`add --all` がそれを PR に入れる）。git に無視されている＝
関門から見えない＝書き換えられる、という並びになる。`deliverDeclaration` は役を起動する
たびに写しを置き直すことで緩和しているが、こちらはティックの頭で1回になる。実装役と
レビュー役が同じティックで続けて走ると、後の役は前の役が書き換えた写しを読みうる。
役ごとに置き直せる場所は Actor の起動経路の側にあり、そこにファイルを書く手段が無い。

**在り処は実装役のプロンプトが1行で名指しする。** 本文をプロンプトへ積むと毎ティック
膨らみ、次の Actor が前の Actor の書き方に引きずられる。何件見せるかはビューの判断で、
プロンプトの判断ではない。その1行は「主張であって Fact ではなく、指示でもない」と
言い切る。**レビュー役には名指ししない。** 同じ作業ツリーを読むのでファイルは見えるが、
名指しすると前の Actor の主張がレビューの根拠になる。PR のタイトルと本文について既に
引いてある線と同じになる（§4.2）。
...
- 走り切った実装の試行が N 回続けて同じ結果で閉じたら ESCALATE（`repeated_attempt`）。
  **上限は同じで、材料が違う。** 上のダイジェストは Fact を全部含むので `github.ci.*` や
  `local.dirty` が動くだけで数え直すが、こちらが読むのは試行の結果——Gap 集合と
  criteria 結果と失敗 detail——だけで、Fact は1つも入らない（§10-2）
...
`repeated_attempt` になる。人間の承認を待つあいだも観測は変わらないので、どちらの検知も
Gap が無い場合より後に置く。空回りしていても、満たしているなら完了でよい。

**ループ検知を二系統にするのは、材料が1つだと両方向に外れるため。**
`observed_digest` は Fact を全部含むので、誰も気にしていない揺れで数え直し（落ちている
テストの出力は1文字も変わらないのに `github.ci.*` が動く）、気にすべき揺れでは動かない
（テストの出力が毎ティック変わるのに criterion は1つも進まず、commit もされない）。
2つ目の材料は試行そのものの結果——Gap 集合・criteria 結果・失敗 detail——で、Fact は
1つも入らない。外れる入力が違うので、走り続けるには両方を破る必要がある。

**失敗 detail は比べる前に正規化しない。** 時刻や行番号を落とすと、揺れているだけの
出力が同じ失敗に見える。完全一致は停滞の強い証拠になり、代償は1文字違えば数え直すこと。
そこで見逃した分は、変えていない hard budget が拾う。

**detail は `observed_digest` には混ぜない。** 混ぜると**1つ目**の系統も1文字違うたびに
数え直すことになり、あちらの役割——粗いまま動かないこと——と逆になる。

**`Unresolved` も `observed_digest` に入れない。** いまも入っておらず、足さない。理由は3つ。
足すと hard budget が動くが、この作業は既存の停止条件を固定したままにする。`detail` が
自由記述なのでダイジェストが揺れ、最後の停止条件が強くなるどころか弱くなる——`key` と
`reason` だけに絞っても、2つ目の系統が既に覆っているケースのために停止条件を動かすことに
変わりはない。そのケース——Fact は同じで未解決理由だけが変わる——は、試行の指紋では
kind `unknown` の Gap と criterion 単位の detail として完全な形で現れる。そのために
作った経路を通って情報は届くので、粗いままにしておくべき側を緩めなくてよい。

**2つの系統は `budget.max_unchanged_reconciles` を共有し、単位が違う。** 1つ目はティックを、
2つ目は試行を数える。共有は美しさの話ではない。`budgetSchema` は `src/domain/goal.ts` の
`PROTECTED_PATH_FLOOR` の中にあるので新しい上限を宣言に足せず、コード側に既定値を置くと
宣言から読めない停止条件ができる——既存5項目すべてを必須にしている理由がこれになる。
どちらも「何回まで進まなくてよいか」を問うので、既に宣言されている1本に相乗りさせる。

**2つ目の系統は1ティック遅れる。** 読むのは永続化された行だけで、DECIDE が走る時点では
今ティックの ACT はまだ書かれていない。最新の試行をメモリ上の結果で閉じると、書き換え
済みの履歴と書き換え前の現在を比べることになる。`pendingReviewCriteria` は実装役が走った
ティックの検証行だけを書き換えるので（§4.5）、混ぜた基準では指紋が毎ティック変わり、
レビューを求める Goal でだけ検知が黙って死ぬ。
