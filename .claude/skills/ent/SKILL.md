...
`.goals/` が無い場所では `ent doctor` の `goals` が failed になる。壊れているのではなく、
まだ始めていない。そこでは最初に1度だけ叩くものがある。

```
ent init                    # .goals/ と .gitignore の行と Goal の雛形を置く
```

冪等で、2度目は既にある `.goals/*.yaml` を上書きせず、`.gitignore` に同じ行を二重に
足さない。git リポジトリでなければ何も作らずに終了コード 1 で断る。

雛形はスキーマとして妥当なところまでしか埋まっていない。`desired_state` と
`acceptance_criteria` は人間が書くもので、**エージェントが埋めて `ent start` まで
進めてよいものではない。** 何を達成するかの宣言にあたる。
